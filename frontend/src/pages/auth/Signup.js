...
const Signup = () => {
  ...
  const checkForm = () => {
    ...
    if(state.password !== state.r_password) {
      setState({
        ...state,
        error: {
          ...state.error,
          r_password: 'password doesn\'t match'
        }
      })
      return false;
    }
      ...
      <RelativeContainer height="fit-content">
        ...
        <FlexContainer>
          ...
          <input
            type="password"
            name="r_password"
            value={state.r_password}
            onChange={valueChanged}
          />
        </FlexContainer>
        <FlexContainer margin="0 0 20px 0">
          {state.error.r_password && <p style={{ margin:0, fontSize:'13px', fontStyle:'italic', color:'red' }}>{state.error.r_password}</p>}
        </FlexContainer>
        <FlexContainer margin="0 0 10px 0">
          <button onClick={singup}>Signup</button>&nbsp;&nbsp;&nbsp;
          <button onClick={ e => goto('/signin')} >Already have one!</button>
        </FlexContainer>
      </RelativeContainer>
    </FlexContainer>
  )
